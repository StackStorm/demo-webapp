...
function createError(transaction, exception, customParameters, config) {
  ...
  if (transaction) {
    ...
    for (var i = 0, l = keys.length; i < l; ++i) {
      params.userAttributes[keys[i]] = custom[keys[i]]
    }
  }

  if (transaction && transaction.url) {
    var url = transaction.url
    var statusCode = transaction.statusCode || 500
    ...
    if (config.capture_params) {
      ...
      for (var i = 0, l = urlKeys.length; i < l; ++i) {
        params.agentAttributes[urlKeys[i]] = urlParams[urlKeys[i]]
      }

      var reqKeys = Object.keys(reqParams)
      for (var i = 0, l = reqKeys.length; i < l; ++i) {
        params.agentAttributes[reqKeys[i]] = reqParams[reqKeys[i]]
      }
    }
  }
  ...
  if (customParameters && !isHighSec) {
    ...
    var customKeys = Object.keys(customParameters)
    for (var i = 0, l = customKeys.length; i < l; ++i) {
      if (ignored.indexOf(customKeys[i]) === -1) {
        params.userAttributes[customKeys[i]] = customParameters[customKeys[i]]
      }
    }
  }
